...
class load_US_data:
    def __init__(self, file_name, scanner = 0):
        ...
        del data
        self.peak_to_peak = self.data_4D - self.data_4D.min(axis = -1, keepdims = True)
    ...
    def calc_PII(self, pulse_average = False):
        if not pulse_average:
            offset = np.where(self.peak_to_peak[self.max_idx] > 1.5 * self.peak_to_peak[self.max_idx].mean())[0][0]
    ...
    def plot_XY(self, param, slice_idx = "max_slice", save_figures = False):
        if param.lower() == "pressure":
            plot_val = (self.peak_to_peak.max(axis = -1, keepdims = True).squeeze() / self.conversion_factor)
        elif param.lower() == "intensity":
            plot_val = (self.peak_to_peak.max(axis = -1, keepdims = True).squeeze() / self.conversion_factor) **(2) / self.Z0
        
        else:
            print("Error! Unrecognized parameter type. Please enter 'Pressure', or 'Intensity'")
            return
